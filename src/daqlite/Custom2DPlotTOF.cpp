...
Custom2DPlotTOF::Custom2DPlotTOF(Configuration &Config, ESSConsumer &Consumer,
                           Projection Proj)
...
void Custom2DPlotTOF::setCustomParameters() {
...
QCPColorGradient Custom2DPlotTOF::getColorGradient(std::string GradientName) {
...
std::string Custom2DPlotTOF::getNextColorGradient(std::string GradientName) {
  bool SaveFirst{true};
  bool SaveNext{false};
  std::string RetVal;
...
void Custom2DPlotTOF::clearDetectorImage() {
  std::fill(HistogramData.begin(), HistogramData.end(), 0);
  plotDetectorImage(true);
}

void Custom2DPlotTOF::plotDetectorImage(bool Force) {
...
void Custom2DPlotTOF::updateData() {
  auto t2 = std::chrono::high_resolution_clock::now();
  std::chrono::duration<int64_t, std::nano> elapsed = t2 - t1;

  // update histogram data from consumer of worker thread
  std::vector<uint32_t> Histogram = mConsumer.readResetHistogram_2D_TOF();
...
void Custom2DPlotTOF::showPointToolTip(QMouseEvent *event) {
